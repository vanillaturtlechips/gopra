...
import './StaggeredMenu.css'; // ❗️ CSS 임포트
...
export interface StaggeredMenuProps {
  ...
  className?: string;
  githubUrl?: string; // ❗️ 로고 대신 깃허브 URL
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // ❗️ [수정] <a> 태그 클릭 시 스크롤 + 메뉴 닫기 핸들러
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, link: string) => {
    e.preventDefault();
    
    // 외부 링크(깃헙 등)가 아닌 경우에만 스크롤
    if (link.startsWith('#')) {
      const targetId = link.substring(1); // #about -> about
      const targetElement = document.getElementById(targetId);
      
      if (targetElement) {
        const headerOffset = 0; // 새 메뉴는 높이가 0이므로 오프셋 필요 없음
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          {/* ❗️ [수정] 깃허브 아이콘으로 로고 변경 ❗️ */}
          <a 
            href={githubUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="sm-logo-text" // (스타일은 텍스트 로고와 동일하게 사용)
            style={{ fontSize: '1.75rem' }} // 아이콘 크기 조절
          >
            {/* GitHub SVG 아이콘 */}
              ...
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <a 
                    ...
                    data-index={idx + 1}
                    onClick={(e) => handleLinkClick(e, it.link)} // ⬅️ 수정된 핸들러
                  >
